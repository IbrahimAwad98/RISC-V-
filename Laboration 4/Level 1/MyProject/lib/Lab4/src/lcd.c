...
{
	while (r != w)
		LCD_WR_Queue(); // Blocks while emptying the queue
}
...
{
	while (((w + 1) % 256) == r)
		LCD_WR_Queue(); // If buffer full then spin...
...
void LCD_ShowChinese(u16 x, u16 y, u8 index, u8 size, u16 color)
{
	...
	for (j = 0; j < size1; j++)
	{
		...
		{
			if ((*temp & (1 << i)) != 0)
			{						// Read from the lower bit of the data
...
void LCD_Fill(u16 xsta, u16 ysta, u16 xend, u16 yend, u16 color)
{
	...
	{
		for (j = xsta; j <= xend; j++)
			LCD_WR_DATA(color); // Set cursor position
	}
}
...
void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2, u16 color)
{
	...
	if (delta_x > delta_y)
		distance = delta_x; // Pick basic incremental axis
	else
		distance = delta_y;
...
void LCD_ShowChar(u16 x, u16 y, u8 num, u8 mode, u16 color)
{
	...
	if (x > LCD_W - 8 || y > LCD_H - 16)
		return;									  // Outside of display area
...
{
	char text[50];
	// Om percentageText är "0%%" så blir den formaterade strängen "Battery Percentage: 0%"
	sprintf(text, "Battery Percentage %s", percentageText);
	LCD_ShowStr(10, 10, text, WHITE, TRANSPARENT);
}
